...
from PyQt6.QtCore import Qt, QMutex, QMutexLocker, pyqtSignal, QRunnable, QObject, QThreadPool
...
class MaxBridgeAssistant(QWidget):
    ...
    def prepare_library(self):
        ...
        2. Clearing and creating the plugin folder.
        3. Removing the plugin_versions_12.json file if it exists.
        4. Extracting the plugin ZIP file to the plugin folder.
        5. Replacing the MS_API.py file in the extracted folder.
        6. Replacing or copying plugin_versions_12.json.
        7. Creating Quixel.ms files.
        """
        ...
        try:
            ...
            # Step 1: Ensure the local AppData attributes are correct
            ...
            plugin_versions_file = Path(self.megascans_library) / "support" / "plugins" / "plugin_versions_12.json"
            zip_file = Path(self.temp_folder) / f"{self.plugin_version}.zip"
            downloaded_api_path = Path(self.temp_folder) / "MS_API.py"
            destination_api_path = plugin_path / "MSLiveLink" / "MS_API.py"
            downloaded_versions_path = Path(self.temp_folder) / "plugin_versions_12.json"

            # Step 2: Clear plugin folder
            ...
            # Step 3: Remove plugin_versions_12.json if it exists
            self.log_message(f"Removing: {plugin_versions_file}")
            QApplication.processEvents()
            if plugin_versions_file.exists():
                plugin_versions_file.unlink()

            # Step 4: Extract the ZIP file to the plugin folder
            self.log_message(f"Extracting ZIP file to: {plugin_path}")
            QApplication.processEvents()

            plugin_path.mkdir(parents=True, exist_ok=True)

            if zip_file.exists() and zip_file.suffix == '.zip':
                try:
                    with zipfile.ZipFile(zip_file, "r") as zip_ref:
                        zip_ref.extractall(plugin_path)

                    self.log_message(f"Successfully extracted ZIP file to: {plugin_path}")

                except zipfile.BadZipFile:
                    self.log_message(f"Error: The file {zip_file} is not a valid ZIP file.")
                except Exception as e:
                    self.log_message(f"Error extracting ZIP file {zip_file}: {e}")
            else:
                self.log_message(f"Error: ZIP file {zip_file} not found or not a valid ZIP file.")

            # Step 5: Replace MS_API.py in the extracted folder
            self.log_message(f"Replacing MS_API.py in: {plugin_path}")
            QApplication.processEvents()
            if downloaded_api_path.exists():
                try:
                    destination_api_path.parent.mkdir(parents=True, exist_ok=True)

                    shutil.copy2(downloaded_api_path, destination_api_path)
                    self.log_message(f"Successfully replaced MS_API.py in: {destination_api_path}")
            ...
            # Step 6: Replace or copy plugin_versions_12.json
            self.log_message(f"Replacing plugin_versions_12.json in: {plugin_versions_file}")
            QApplication.processEvents()
            if downloaded_versions_path.exists():
                try:
                    destination_dir = plugin_versions_file.parent
                    destination_dir.mkdir(parents=True, exist_ok=True)

                    shutil.copy2(downloaded_versions_path, plugin_versions_file)
                    self.log_message(f"Successfully replaced plugin_versions_12.json in: {plugin_versions_file}")
            ...
            # Step 7: Create Quixel.ms files after library setup
